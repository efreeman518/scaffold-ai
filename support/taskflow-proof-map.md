...
| Phase 5b runtime wiring | `src/Host/TaskFlow.Api`, `src/Host/TaskFlow.Bootstrapper`, `src/Host/Aspire/AppHost`, `src/Host/TaskFlow.Bootstrapper/Registration/ProviderSwitchAttribute.cs`, `src/Application/TaskFlow.Application.Contracts/Configuration/HostingLaneSelector.cs`, `src/Host/TaskFlow.Bootstrapper/StartupTasks/EnsureExternalResources.cs` | Middleware order, independent provider-switch registration, lane-default resolution, one-time external resource provisioning, AppHost resources, runtime config, and deployment shape. |
...
| Phase 5d deployment and rollback | `.github/workflows/deploy.yml`, `.github/workflows/deploy-vps.yml`, `deploy/compose/docker-compose.yml`, `infra/scripts/Test-ReleaseManifest.ps1`, `infra/scripts/Invoke-DeploymentSmoke.ps1`, `tests/Test.Unit/Infrastructure/DeploymentWorkflowContractTests.cs`, deployment Dockerfiles | Azure and Portable lanes, validated green SHA, build-once immutable release manifests, digest-pinned images, artifact-backed deploy/rollback without rebuild, database-first and public readiness, CRUD smoke cleanup, queued deployments, BuildKit-only package credentials, and non-root chiseled runtime images. |
...
- **Provider and lane selectors:** `tests/Test.Architecture/ProviderSwitchArchitectureTests.cs` proves default-arm DI resolution and cloud-SDK boundaries. `tests/Test.Aspire/AppHostLaneTopologyTests.cs` proves lane precedence, unknown-value failure, host environment, and Azure/Portable topology without requiring a running DCP graph.
...
- **Portable deployment:** `deploy/compose/docker-compose.yml` and `.github/workflows/deploy-vps.yml` are configuration/deployment proof. Treat runtime behavior as `deployment-only` until the full Compose smoke executes successfully in an eligible lane.
