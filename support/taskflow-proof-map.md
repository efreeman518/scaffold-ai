...
| Phase 5b runtime wiring | `src/Host/TaskFlow.Api`, `src/Host/TaskFlow.Bootstrapper`, `src/Host/Aspire/AppHost`, `src/Shared/TaskFlow.Hosting/HostingLane.cs`, `src/Host/TaskFlow.Bootstrapper/Registration/ProviderSwitchAttribute.cs`, `src/Host/TaskFlow.Bootstrapper/StartupTasks/EnsureExternalResources.cs` | Middleware order, strict Azure/NonAzure provider selection, one-release Portable and Relational aliases, lane-default resolution, one-time external resource provisioning, AppHost resources, runtime config, and deployment shape. |
| Phase 5b Aspire service defaults | `src/Host/Aspire/ServiceDefaults` | OpenTelemetry wiring, `/healthz/live`, `/healthz/ready`, and `/healthz` contracts, shared host registration. |
...
| Phase 5c scheduler | `src/Host/TaskFlow.Scheduler`, `src/Infrastructure/TaskFlow.Infrastructure.Data/Operational/OperationalWorkBase.cs` | TickerQ registration, real idempotent jobs, bounded workers, lease-based operational work, and retention handlers. |
...
| Phase 5d deployment and rollback | `.github/workflows/deploy.yml`, `.github/workflows/deploy-vps.yml`, `deploy/compose/docker-compose.yml`, `infra/scripts/Test-ReleaseManifest.ps1`, `infra/scripts/Invoke-DeploymentSmoke.ps1`, `tests/Test.Unit/Infrastructure/DeploymentWorkflowContractTests.cs`, deployment Dockerfiles | Azure and NonAzure lanes, validated green SHA, build-once immutable release manifests, digest-pinned images, artifact-backed deploy/rollback without rebuild, database-first and public readiness, CRUD smoke cleanup, queued deployments, BuildKit-only package credentials, and non-root chiseled runtime images. |
...
- **Provider and lane selectors:** `src/Shared/TaskFlow.Hosting/HostingLane.cs` owns the shared strict resolver. `tests/Test.Architecture/ProviderSwitchArchitectureTests.cs` proves default-arm DI resolution and cloud-SDK boundaries. `tests/Test.Aspire/AppHostLaneTopologyTests.cs` proves lane precedence, unknown-value failure, the deprecated Portable alias, PostgreSQL JSONB and MongoDB read-model arms, host environment, and Azure/NonAzure topology without requiring a running DCP graph.
...
- **NonAzure deployment:** `deploy/compose/docker-compose.yml` and `.github/workflows/deploy-vps.yml` are configuration/deployment proof. Treat runtime behavior as `deployment-only` until the full Compose smoke executes successfully in an eligible lane.
